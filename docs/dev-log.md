...
3. `west init` SDK manifests at `C:\mcxn\mcuxsdk-ws` on tag/rev `v26.06.00-LTS` (manifest commit `b01ab903…`).
...
- SEC 26.06 / `securep.exe` **not installed** (NXP account download required) — needed before P4.
- JP13/R274 not visually confirmed yet — will prove via stock ENET example.
...
## 2026-09-03 — Phase P0 progress + Ethernet gate

### Completed

1. SDK examples available under `C:\mcxn\mcuxsdk-ws\mcuxsdk` (west update still may be finishing optional repos).
2. Built + flashed `hello_world` — UART `hello world.` / SDK `2026.06.00`.
3. Built + flashed `freertos_hello` — UART `Hello world.`.
4. Built `lwip_ping_freertos`, `ota_mcuboot_server_enet`, `ota_mcuboot_basic`, `mcuboot_opensource` (all link OK).
5. Flashed `lwip_ping_freertos` — **PHY Auto-negotiation failed** (hardware gate).
...
**P0.2 / §1.5 Ethernet hardware prerequisite not satisfied.** Host Ethernet adapters disconnected; board PHY reports no link. JP13/R274 not visually verified.

### Next (at Ethernet gate)

Resolved by user: Ethernet connected; static IP policy **192.168.2.90/24**.

## 2026-09-03 — P0 closed; P1/P2 progress; P3 CMPA gate
...
### P2
- Freestanding export of `ota_mcuboot_server/enet` → `firmware/app`.
...
- Verified: Hello → `Hello PC!`; STATUS; IP 192.168.2.90; UUID stable; update port open in window.
- V1 and V2 images build.

### Next gate (P3)
- First CMPA `BOOT_SRC` write + IFR MCUboot program requires **owner approval**.

## 2026-09-03 — P3 APPROVED; ISP transport gate
...
2. **CMPA/CFPA binary backups** (debug `mem-tool` AHB read; validated by reading app flash @0x0 successfully):
   - `C:\mcxn-secrets\DEV-UNIT-01\backup\cmpa_pre_p3.bin` — 512 B, all `0xFF` (erased/unprogrammed)
     - SHA-256: `9f56cda75fefeab90f6fa5d5ddc9601544b121732c5ecccab32e631060453a5d`
   - `C:\mcxn-secrets\DEV-UNIT-01\backup\cfpa_pre_p3.bin` — 512 B; almost all `0xFF`, trailer `0x1F0..0x1FF` = `b4a2ee48ab01e4ca9e3f304f15bc9bcd`
...
3. **Inspected NXP SDK `cmpa.bin`** (`…\mcuboot_opensource\cmpa.bin`):
   ...
   - Security fields (lifecycle/debug-lock/secure-boot/RoTKH/NPX/CUST_MK_SK/seal/CRC) remain default/disabled/zero — **no unexpected security changes**
...
Observed:
- After DM `ispmode`, **no** USB device `VID_1FC9/PID_014F` (HS USB **J11** not presenting ROM HID — cable likely only on MCU-Link **J17**)
...
**A.** Plug USB-C PC↔**J11** (HS USB), keep J17; reply `USB ISP READY`  
**B.** Hold **SW3**, press/release **SW1**, release SW3; reply `ISP READY`  

Then agent resumes: blhost backup verify → erase/program IFR MCUboot → write `cmpa.bin` → rebuild/sign V1 (`--pad --confirm`) → verify chain → prove recovery.

## 2026-09-03 — P3 USB ISP: J11 connected but HID not enumerating

Owner replied `USB ISP READY` (J11 + J17).

### Observed
- DM `ispmode` succeeds (modes 0/4/5 tried).
- After ISP, Windows shows **`Unknown USB Device (Device Descriptor Request Failed)`** `USB\VID_0000&PID_0002\…` — J11 is electrically active.
...
4. Verified ROM → IFR MCUboot → V1; ping; Hello; STATUS.
...
### Next gate
**P5** Ethernet SB3 — owner accepted P4; continue autonomously.

## 2026-09-03 — P5 complete
